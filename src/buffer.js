...
export class StringBuffer {
    ...
    if (size > 1) {
      ...
      for (let i = 1; i < size; i++) {
        ...
        if (line.slice(0, 1) === "\\") {
          ...
          let indentationNumber = parseInt(indent)
  ...
  appendStringBufferString(stringBuffer) {
    ...
    let thisAtoms = this.atoms
    let thisLength = thisAtoms.length
    let stringBufferAtoms = stringBuffer.atoms
    let stringBufferLength = stringBufferAtoms.length
